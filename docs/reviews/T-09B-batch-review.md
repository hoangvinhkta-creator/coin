...
Ba file runtime MỚI mà bảng khai báo chưa có (xem `F-T09B-05`/`H-32`): `webapp/firebase_config.js`
...
ghi lại khi có sự kiện `online`. Phần còn hở → `F-T09B-03`/`H-30`.

### F-T09B-02 — Trần 1 MiB/document Firestore với `ethdca/state` (ledger tăng không giới hạn)
Phân loại: **HARDENING (PROVISIONAL)** → `H-29`
...
Phân loại: **HARDENING (CONFIRMED)** → `H-30`
...
Phân loại: **HARDENING (CONFIRMED)** → `H-31`
...
Phân loại: **HARDENING (CONFIRMED, tầng governance)** → `H-32` · Capability: `CAP-GOVTOOL` · Owner: chưa có
...
## 6. Verdict
    ...
    PROVISIONAL                    = 1   (F-T09B-02 -> H-29)
    HARDENING                      = 4   (F-T09B-02 -> H-29, F-T09B-03 -> H-30,
                                          F-T09B-04 -> H-31, F-T09B-05 -> H-32)
