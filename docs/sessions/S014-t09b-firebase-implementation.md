...
- Batch review: `docs/reviews/T-09B-batch-review.md` — 1 finding CONFIRMED sửa cùng lượt
  (`F-T09B-01`, hai tab stale ghi đè → transaction có điều kiện rev), 4 HARDENING (`H-24..H-27`).
- State canonical: Task Spec (Status + 16 evidence), `PROJECT_PROGRESS.md`, `REVIEW_BUDGET_LEDGER.md`
  §2.2 hàng `0'` + §2.2.4, `CAPABILITY_REGISTRY.md` §10, `HARDENING_BACKLOG.md` H-24..H-27,
...
- **`validateState()`** = schema + kiểu + không âm + bất biến bảo toàn (contribution = Σbase + Σsmart
  + oppAdded; oppFund = Σ oppAdded) — KHÔNG backfill, KHÔNG suy luận `month` (H-26 ghi giả định).
...
- `H-24` (trần 1 MiB), `H-25` (stale change chỉ trong bộ nhớ), `H-26` (giả định tổng tỷ lệ = 1 khi
  validate), `H-27` (production paths chưa khai ba file mới) — HARDENING có re-trigger.
...
file runtime mới vào `PRODUCTION_PATHS.md` (`H-27`, gom với `H-12`/`H-21`).
...
- `PROJECT/HARDENING_BACKLOG.md` H-23..H-27
