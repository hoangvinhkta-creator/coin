...
hội chạm bất kỳ logic nào khác. Ghi HARDENING (`H-18` — renumber từ `H-16` do đụng độ ID với
DATA stream khi hợp nhất, xem `PROJECT/HARDENING_BACKLOG.md`) — không phải BLOCKING, vì không có đường production nào cho
...
- `PROJECT/PROJECT_PROGRESS.md` — RSK-003, RSK-004, roadmap table (T-03/WP-C1/T-09A), Session
  History
- `PROJECT/HARDENING_BACKLOG.md` — thêm H-18 (ghi chú kiến trúc cho V-03; renumber từ H-16
  gốc do đụng độ ID với DATA stream khi hợp nhất vào `main`)
...
- **V-03 BÁC BỎ** về hành vi quan sát được (mọi trạng thái INVALID dựng được đều bị chặn tạo
  ...
  `engine.js`, không do kiểm tra `data_quality` tường minh — ghi H-18 (HARDENING; renumber từ
  H-16 gốc do đụng độ ID với DATA stream khi hợp nhất vào `main`).
