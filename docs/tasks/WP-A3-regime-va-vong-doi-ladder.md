...
## Subtasks
- [ ] A3.1 Tách nhãn dẫn xuất STRESSED khỏi trạng thái nền NORMAL/CRASH/RECOVERY, hoặc phương án
      tương đương đạt cùng bất biến; ghi quyết định thiết kế
- [ ] A3.2 Đóng đường ra của Crash ladder cho **mọi** trạng thái kết thúc Recovery, không riêng NORMAL
- [ ] A3.3 Chặn regime exit dựa trên dữ liệu thiếu; `None` không được coi là 0.0
- [ ] A3.4 Sửa snapshot [F5] đo theo AVAILABLE đã unlock; daily limit áp ở khâu deployment
- [ ] A3.5 Gắn nhãn pool của Crash zone theo nguồn vốn thật cho tie-break §15.1
- [ ] A3.6 Viết test vòng đời CRASH → RECOVERY → STRESSED
- [ ] A3.7 Viết test [F1] phủ đủ năm bề mặt: unlock, ladder, cooldown, limit, execution
- [ ] A3.8 Định lượng thay đổi kết quả mô phỏng trước–sau trên cùng seed/dataset
- [ ] A3.9 Rà soát toàn diện: mọi trạng thái ladder đều có đường kết thúc giải phóng reserve
...
- (dự kiến) `docs/reviews/E2-WP-A3-*.md`

Modified:
- (dự kiến) `src/eth_dca_os/regime.py`, `engine.py`, `ladders.py`
- (dự kiến) `tests/`, có thể `docs/CONVENTIONS.md`
