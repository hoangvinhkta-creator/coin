...
| PLANNED | T-06 | Chạy backtest chính thức trên dữ liệu thật | Mở cổng verdict — đây là đường găng tới mục tiêu cuối | C | xhigh | `T-06 = GATE-A ∧ BLK-001` (`RCP-002` điểm 9, đã áp dụng — **T-05 KHÔNG phải điều kiện**, xác nhận `DEC-029`). (A) nội tại — **GATE-A** (WP-A1…**WP-A7** đều DONE — **GATE-A = CLOSED tại `DEC-028`, 2026-09-03**); (B) hạ tầng — BLK-001 (mạng Binance). NOT READY: chờ (B) BLK-001 gỡ |
...
2026-09-03). T-06 chỉ còn cần: BLK-001 (hạ tầng) gỡ. **T-05 KHÔNG phải điều kiện của T-06**
(`RCP-002` điểm 9, đã áp dụng; xác nhận `DEC-029` — đóng inconsistency trước đó giữa dòng T-05
và dòng T-06 của chính bảng này).
T-06 = GATE-A ∧ (BLK-001 đã gỡ)
```
