...
không phân biệt trực quan theo loại giao dịch. **Thuần trình bày — `webapp/ledger.js` diff
RỖNG**, không đổi một field/phép tính nào của `derive()`. `#dashBottom` nay 3 khối có tiêu đề
("Đang nắm giữ" / "Tiền mặt & USDT" / "Lãi/lỗ đã thực hiện", holdings rỗng hiện "Chưa có coin
nào"), lưới 2 cột ở >860px / 1 cột ở ≤860px (khớp breakpoint `.hero` sẵn có). Bộ lọc Lịch sử đổi
sang nút `.histtype` (tái dùng style `.txtype`) có đếm số lượng mỗi loại trên toàn bộ `s.events`;
`#histFilterType` (select) **giữ nguyên id**, chuyển ẩn, đồng bộ giá trị — không cần sửa một dòng
test nào vì grep xác nhận `test_t12_browser.js`/`test_stepb_ui.js` không thao tác bốn id lọc lịch
sử. Mỗi loại giao dịch có icon/màu riêng cạnh `hc-date` (BUY xanh lá "↑", SELL đỏ "↓", CASH xanh
dương "$" với biến CSS mới `--cash`/`--cash-bg`, TREASURY xám "⇄", PRICE viền nhạt "≈"; RESERVE
**giữ nguyên không đổi** theo đúng yêu cầu) — không xoá badge/chip cũ nào. Completion Gate
**14/14 REQUIRED PASS** (E1 toàn bộ — task không chạm `accounting_financial` nên không mở vòng
E2 độc lập); toàn bộ `npm test` (10 suite, gồm Firestore Emulator + Chromium thật) **PASS
nguyên văn, không sửa một dòng test nào**; bốn phép đo trực tiếp trên trình duyệt thật (tiêu đề
nhóm, đếm bộ lọc, class icon, `getComputedStyle` layout 2↔1 cột) xác nhận đúng yêu cầu — kịch bản
đo không commit vào repo (đúng ngân sách artifact). Một lỗi thứ tự CSS tự phát hiện và tự sửa
trong phiên (rule override mobile ban đầu bị rule base 2-cột đè do đứng trước nó trong nguồn),
bắt được nhờ đo `getComputedStyle` thật thay vì chỉ đọc CSS. `CAP-WEBAPP`: `T-17` tiêu **0** repair
cycle (implementation ban đầu), budget **KHÔNG đổi `4/2/2`**, không cần `OWNER_EXTENSION`. Không
`HARDENING` mới. `H-41` giữ nguyên: chưa nên dùng tiền thật. Chi tiết:
`docs/reviews/T17-IMPLEMENTATION-AND-E2-REPORT.md`.

Trước đó, 2026-09-06 — **`S043` — `T-16` (sổ đa tài sản, SELL đúng, CASH): `NOT_PLANNED → DONE` trong một
...
nghĩa restore/derive), `npm --prefix webapp test` exit 0 chạy lại trong môi trường của reviewer.
Completion Gate **12/12 REQUIRED PASS** (E1+E2). 0 finding `BLOCKING`. `H-42` phần REQUIRED
**ĐÓNG**; `H-49` **ĐÓNG** (mô tả chữ ký lỗi legacy sửa theo `F-T14-E2-01`); `H-51`/`H-52` xác
nhận độc lập, giữ nguyên HARDENING (`H-52` KHÔNG resolve); một finding mới `H-53`
(`F-T14-E2-03`). `CAP-WEBAPP` budget **KHÔNG đổi** `2/1/1` — E2 và closure không tiêu repair
cycle nào (production diff của cả hai phiên = EMPTY). **Guard SELL/real-money giữ nguyên**:
`T-14 DONE` KHÔNG cấp phép SELL thật (`H-46` vẫn ACTIVE), KHÔNG có nghĩa sẵn sàng tiền thật
không giới hạn (`H-41` vẫn còn hiệu lực), bước D (`OWNER_LOCAL_ACCEPTANCE`) vẫn CHƯA mở.
**Integration Decision**: `branch_authority_check` báo `INTEGRATION_DECISION_REQUIRED: loc>5000`
(artifact review/evidence, production diff EMPTY) — Owner quyết định **ACCEPT
INTEGRATION/MERGE** (`DEC-050` §E), ghi nhận nhưng **chưa** thi hành merge `main`. Chi tiết đầy
đủ: `docs/reviews/T14-E2-INDEPENDENT-REVIEW.md`, `docs/reviews/T14-OWNER-CLOSURE.md`.

Trước đó, 2026-09-06 — **`S037` — Independent E2 review + Owner-authorized Lifecycle Closure: `T-13:
...
để lại tại `docs/reviews/T12-OWNER-CLOSURE.md` §7 ("Step B là quyết định Owner, không phải hệ quả
tự động của `T-12 DONE`"). Sản phẩm của phiên: `docs/spec-l1/COINDCA_L1_STEP_B_UX_SPEC.md`
(`CANONICAL — APPROVED`, mới) đặc tả IA 4 điểm đến, Dashboard Contract §16 áp dụng, 8 loại sự
kiện nhập liệu, lịch sử có filter, sửa/xoá an toàn, plan/carry UX, UNKNOWN UX, mobile workflow, và
12 kịch bản chấp nhận (AS-01…AS-12); `docs/tasks/T-13-buoc-b-dashboard-giao-dich-lich-su.md`
(mới) định nghĩa Scope IN/OUT, Expected Touch Area, 13 REQUIRED check Completion Gate. Task ID
mới = **1** (`T-13`); capability `CAP-WEBAPP`, lineage root `WP-C1` — KHÔNG tạo capability/lineage
mới, budget KHÔNG đổi (`allowed 2 / used 1 / remaining 1`). SELL vẫn bị ẩn hoàn toàn khỏi Step B
(`H-46`/`F-E2-03` chưa có Owner Decision xử lý); Firebase/auth/rules/Hosting KHÔNG bị chạm (bước
C, `H-42` vẫn mở). Production diff = **EMPTY**. Không thi hành — task dừng ở `READY`, KHÔNG
chuyển `IN_PROGRESS`. Chi tiết: `docs/sessions/S035-l1-step-b-task-definition.md`.

Lịch sử cập nhật trước:
2026-09-05 — **T-12: IMPLEMENTED → DONE** (`DEC-046`, Owner-authorized Lifecycle Closure) sau
independent E2 `E2_VERDICT = PASS` (`docs/reviews/T12-E2-INDEPENDENT-REVIEW.md`). 9/9 check
E2-required PASS trên bằng chứng độc lập tái lập được (oracle WAC riêng, property test bảo toàn
VND, 300 hoán vị + 6 TZ, 14 mutant độc lập, production reachability riêng trên `app_final.html`),
0 BLOCKING. Completion Gate **14/14 PASS**. Golden `c610a29` không đổi trong suốt E2/closure;
production diff của cả E2 lẫn closure = RỖNG. `CAP-WEBAPP` budget KHÔNG đổi **2/1/1**,
`REPAIR_CYCLE_1` vẫn CONSUMED, không tiêu chu kỳ thứ hai. Bốn finding E2 route thành HARDENING
`H-44`…`H-47` (không task mới). **`T-12 DONE` KHÔNG cấp phép dùng tiền thật cho SELL** — `H-46`
(`F-E2-03`, khiếm khuyết đặc tả) phải xử lý TRƯỚC khi mở nghiệp vụ SELL thật; `H-42`
(Firebase isolation/xác thực bền vững) và ràng buộc product-readiness khác giữ nguyên.

Lịch sử cập nhật trước:
2026-09-05 — S034: T-12 **IMPLEMENTED**, independent E2 REQUIRED. DEC-045 đã disposition
hai nhóm trước implementation; Ready Gate 17/17. SC12/12, INV15/15, mutation7/7 và
P1…P6 PASS E1; full Python678/678 PASS, npm baselinePASS/sau exit1 ở luồng legacy đã gỡ,
22phạm vi N/A có neo từng ca và persistence còn áp dụng đã kiểm lại. Golden`c610a29`,
repairHEAD`2a2ab3f`, REPAIR_CYCLE_1 CONSUMED, pool2/1/1. Không DONE hay mở dùng tiền thật.

Lịch sử cập nhật trước:
2026-09-05 — S034 tiếp nối DEC-045: hai nhóm mâu thuẫn đã disposition. Tái đánh giá Ready Gate
duy nhất 17/17 PASS; T-12 BLOCKED → READY → IN_PROGRESS. Bắt đầu implementation đã ủy quyền;
không broad preflight mới. Golden UNFROZEN, repair NOT_CONSUMED, independent E2 REQUIRED.
...
vẫn thoả. Ba tu chỉnh: (1) sổ `coindca.ledger/2` được phép thay thế/tiến hoá state đã lưu **bên
trong** ranh giới `ethdca/state` đã có — KHÔNG collection/document mới, KHÔNG sửa
`firestore.rules`, KHÔNG đổi kiến trúc Firebase; (2) `T12_GOLDEN_ACCOUNTING_BASELINE` (tổng hợp,
`SC-01`…`SC-12`) tách khỏi `GOLDEN_BASELINE_SHA` tầng dự án (`H-10`, lineage `T-06`, KHÔNG phải
phụ thuộc của `T-12`) và khỏi `OWNER_LOCAL_ACCEPTANCE` (dữ liệu thật, bước D); (3) MỘT repair
cycle pre-authorized cho `T-12`, RÚT TỪ pool `CAP-WEBAPP` hiện có (`allowed 2 / used 0 / remaining
2`, không cộng thêm), dùng được chỉ khi đủ điều kiện đã liệt kê (§ Budget review/repair). Không
task ID mới. Production diff = **EMPTY**. Chi tiết: `PROJECT_DECISIONS.md` `DEC-043`,
`docs/tasks/T-12-so-cai-l1-v2-va-derive.md`, `docs/sessions/S033-t12-owner-amendments-dec043.md`.

Trước đó, cùng ngày — 2026-09-05 — **`S032` — MỞ `T-12` (CoinDCA L-1 bước A): Sổ cái L-1 v2 + `derive()` tất định +
migration + test kế toán. Trạng thái `NOT_PLANNED → READY`, Completion Gate FROZEN, Tier D /
Effort max.** Thẩm quyền: `DEC-042` § Consequence (*"việc mở task ID cho bước A thuộc một phiên
riêng sau `DEC-042`"*). Task ID mới = **1** (`T-12`); registry 22 → 23 task file, 29 → 30 roadmap
ID. Capability `CAP-WEBAPP`, lineage root `WP-C1` — KHÔNG tạo capability/lineage mới, KHÔNG reset
budget (`allowed 2 / used 0 / remaining 2`, `DEC-018`), KHÔNG tự cấp repair budget cho `T-12`.
Production diff = **EMPTY**. Không thi hành ledger/migration/UI/Firebase/auth. Chi tiết:
`docs/reviews/L1-STEP-A-TASK-DEFINITION.md`, `docs/sessions/S032-l1-step-a-task-definition.md`.

Trước đó, cùng ngày — 2026-09-05 — **OWNER DECISION `DEC-042` — DUYỆT CoinDCA L-1 Product +
Accounting Spec** (`docs/spec-l1/COINDCA_L1_PRODUCT_ACCOUNTING_SPEC.md` = `CANONICAL — APPROVED`;
bốn quyết định kế toán `OD-L1-1`…`OD-L1-4` đã chốt). Không task ID mới tại `DEC-042`.

Trước đó, cùng ngày — 2026-09-05 — **OWNER DECISION `DEC-040` — `T-07: READY → DONE`, chọn `L-1` (benchmark đơn giản
hơn).** Đây là lần thực thi đầu tiên và duy nhất của `T-07`. Owner đọc
`docs/reviews/T07-OWNER-DECISION-BRIEF.md` và `docs/reviews/T07-RQ-EVIDENCE-INVESTIGATION.md`
§10 (evidence RQ-1..RQ-5), rồi chọn **L-1**: chấp nhận V2.1.5 không tạo được accumulation edge,
hướng đi của dự án quay về một benchmark/DCA đơn giản hơn thay vì tiếp tục chiến lược
timing/reallocation phức tạp dưới V2.1.5. Lý do Owner nêu: evidence replay mới làm YẾU giả
thuyết timing edge ổn định (Control F P95 chỉ 2/9 window, Control G P95 chỉ 3/9, `OOS` thua cả
hai control ở cả median lẫn P95), trong khi capital-allocation mismatch mới ở mức PARTIALLY
ESTABLISHED — chưa đủ để biện minh mở V2.2 ngay.

**Ý nghĩa hẹp của `T-07 = DONE`**: bước DUYỆT đã thực thi, hướng đi = L-1. **KHÔNG có nghĩa**
V2.1.5 PASS, verdict đổi, hay dự án CoinDCA kết thúc — L-1 chính là bằng chứng dự án tiếp tục,
chỉ đổi chiến lược nền. Giữ nguyên tuyệt đối, không đổi một chữ: `T-06 = DONE` (`DEC-031`,
lịch sử); verdict official = `DO_NOT_BUILD`; `V2.1.5 validation = FAILED` (vĩnh viễn);
`can_proceed_to_app = false`; `GATE-B = CLOSED` (`DEC-038`); `DEC-005 = PENDING` (không resolve
ở quyết định này, tiếp tục chặn `T-08`).

`T-11` vẫn `BLOCKED`: dưới V2.1.5 (nay đã đóng lại bằng L-1), điều kiện `verdict=BUILD` không
bao giờ thoả được nữa — `T-11` chuyển từ "blocked, chờ dependency" sang **not-applicable vĩnh
viễn dưới V2.1.5**. Một verdict `BUILD` từ một chiến lược/version khác trong tương lai (nếu có)
phải tự đủ điều kiện từ đầu, không kế thừa trạng thái validation của V2.1.5 — đúng nguyên văn
chỉ thị Owner.

Quyết định này KHÔNG mở `V2.2`/`WP-D2`, KHÔNG mở/thực thi `WP-C3`/`WP-C4`/`T-08`, KHÔNG resolve
`DEC-005`, KHÔNG chọn Objective A/C — Owner tường minh "Do not implement downstream product
changes in this session" và "STOP after T-07 lifecycle closure". Không sửa `src/`/`tests/`/
`webapp/`. Không task ID mới. Production diff = EMPTY. Chi tiết đầy đủ: `DEC-040`
(`PROJECT_DECISIONS.md`).

Trước đó, cùng ngày — 2026-09-05 — **CANONICALIZE Owner-run REPLAY evidence cho RQ-1/RQ-3/RQ-4 (dataset official T-06,
...
băng 2026-08-23 không bị sửa một chữ ở phần yêu cầu).

Việc đã làm, gọn trong ba ý. (1) **Bảy khoảng trống của §21.2/§21.3/§21.4 nay có test chạy trên
`run_engine` THẬT**: Base execute sớm không lặp lại ngày gốc; Month-End Day 25–27 và Day 28;
snapshot Crash [F5] đo SAU cancel/release (phản chứng: 5,8 so với 2,0 nếu đo trước — chênh đúng
3,8 vừa release); không double reservation giữa Smart/Opportunity/Crash, kiểm ở **MỌI nến** chứ
không chỉ cuối run; một/hai/ba/bốn zone bị xuyên trong cùng một nến với trần hai zone mỗi cycle;
`max_zones` áp SAU khi sắp thứ tự §15.1 (kịch bản dựng đúng tình huống hai thứ tự KHÁC nhau, nếu
không mệnh đề này không kiểm được); Smart theo LOW còn Opportunity theo CLOSE; proxy đêm 07:00,
TTL và MISSED; cooldown + override kể cả tần suất trong CRASH; Crash funding unavailable; và một
lưới hồi quy **thường trực** cho [F1] STRESSED đi NGƯỢC CHIỀU phản chứng của WP-A3.
(2) **Không requirement nào rơi vào im lặng**: bảng đối chiếu **31/31** gạch đầu dòng §21 ở
`docs/CONVENTIONS.md`, và một bộ test đối chiếu bảng đó với CHÍNH văn bản spec, kiểm mọi tên test
được viện dẫn có thật, và kiểm chiều ngược lại (test WP-B2 nào cũng phải gắn với một requirement).
(3) **Không một dòng mã sản phẩm nào bị sửa** — đúng ràng buộc "gói này chỉ viết test".

Đo được: **141 ca test mới** (9 + 31 + 5 + 96), full suite **678/678 PASS** (trước gói 537/537),
`git diff b778dc1..HEAD -- src/eth_dca_os webapp pyproject.toml pyproject.lock` **rỗng**. Bất biến
tài chính đo bằng `tests/wp_b3_invariance_tool.py` trên payload chuẩn tắc 3.728.853 byte:
`sha256 3ea7c8d7…` TRÙNG trước–sau (và trùng cả giá trị `WP-B3` đã ghi ở phiên trước — tái lập
được qua phiên).

Sinh hai mục HARDENING, **không tạo task ID nào**: **`H-39`** — hai kịch bản robustness Gate 3 mà
Impl Plan §8 ghi là bắt buộc (behavioral BT §6; stress P2P-unavailable BT §5) KHÔNG có đường chạy
trong pipeline (`GATE3_GRID` không biến thiên hai cờ đó nên cả 114 config đều `OFF/False`); ánh xạ
vào risk đã đăng ký `RSK-007`, định tuyến `OUT_OF_SCOPE` về `CAP-PIPELINE`, KHÔNG sửa ở đây vì
nằm ngoài Expected Touch Area và vì sửa rồi chạy lại Gate 3 chính là "chạy lại để làm đẹp kết quả
official" mà BT §22 / Master Index §6 cấm. **`H-40`** — nhánh "proxy 07:00 vượt TTL" của BT §6
không tới lượt chạy ở TTL baseline 12h (cùng họ `H-36`).

`WP-B2` **CHƯA `DONE`**: `STATE_AUTHORITY.md` quy định `DONE` do chủ dự án ghi (tiền lệ
`WP-B1`/`DEC-034`, `WP-C2`/`DEC-036`, `WP-B3`/`DEC-037`) → **`OWNER_DECISION_REQUIRED`**.
**`GATE-B` VẪN CHƯA MỞ** (đòi `WP-B1 ∧ WP-B2 ∧ WP-B3` đều `DONE`; nay `WP-B1`/`WP-B3` DONE,
`WP-B2` mới `IMPLEMENTED`). `T-07` vẫn `NOT READY`. Giữ nguyên: `DEC-005 = PENDING` (vẫn chặn
...
Chủ dự án chấp nhận bằng chứng Completion Gate đóng băng (8/8 REQUIRED PASS, đầu ra tài
chính/chiến lược bất biến bit-for-bit, production reachability PASS, full suite 537/537 PASS,
0 finding BLOCKING) và uỷ quyền đóng vòng đời. Đóng `F-024` và `F-033`.

Hệ quả downstream, tính lại NGHIÊM NGẶT theo dependency đã khai (không suy luận thêm):
`WP-B1 = DONE` (không đổi), `WP-B2 = READY` (không đổi — không phụ thuộc `WP-B3`),
`WP-C2 = DONE` (không đổi), `WP-C3 = READY` (không đổi). **`GATE-B` VẪN CHƯA MỞ** (đòi cả
`WP-B1 ∧ WP-B2 ∧ WP-B3` đều `DONE`; nay `WP-B1`/`WP-B3` DONE nhưng `WP-B2` mới `READY`, chưa
`DONE` — đúng một mắt xích còn thiếu). `T-07` vẫn `NOT READY`. `WP-B2`/`WP-C3` **CHƯA mở/thực
thi**.

Giữ nguyên: `DEC-005 = PENDING` (vẫn chặn `T-08`); `T-06 = DONE`; V2.1.5 validation = `FAILED`;
verdict lịch sử = `DO_NOT_BUILD`; `can_proceed_to_app = false`. Không production code nào bị
sửa (quyết định thuần state/docs). Không task ID mới. 0 repair cycle tiêu. `H-36`/`H-37`/`H-38`
giữ nguyên HARDENING, không nâng đường găng. Chi tiết đầy đủ: `DEC-037` (`PROJECT_DECISIONS.md`).

Trước đó, 2026-09-04 — **`WP-B3` THỰC THI: `READY → IN_PROGRESS → IMPLEMENTED`** (phiên `S025`, nhánh
...
Hệ quả downstream, ĐÚNG theo dependency đã khai (không suy luận thêm): `WP-B3: BLOCKED → READY`
(Ready Gate của nó chỉ còn đúng một dòng "Dependency WP-C2 DONE", nay thoả); `WP-C3: PLANNED →
READY` (Dependencies chỉ liệt kê T-04 DONE + WP-C2 DONE, nay đủ). `WP-B2` không đổi, giữ
`READY`. **`GATE-B` vẫn CHƯA MỞ** (đòi cả `WP-B1 ∧ WP-B2 ∧ WP-B3` đều `DONE`; nay `WP-B1` DONE,
`WP-B2`/`WP-B3` mới READY, chưa DONE). `T-07` vẫn NOT READY. Cả `WP-B3` và `WP-C3` **CHƯA mở/
thực thi** — chuyển READY chỉ công nhận tiền đề đã thoả.

Giữ nguyên: `DEC-005 = PENDING` (vẫn chặn `T-08`); `T-06 = DONE`; V2.1.5 validation = `FAILED`;
verdict lịch sử = `DO_NOT_BUILD`; `can_proceed_to_app = false`. Không production code nào bị
sửa (quyết định thuần state/docs). Không task ID mới. 0 repair cycle tiêu. Chi tiết đầy đủ:
`DEC-036` (`PROJECT_DECISIONS.md`).

Trước đó, cùng ngày — **`WP-C2` THỰC THI: `READY → IN_PROGRESS → IMPLEMENTED` (phiên `S024`, nhánh
...
3. **`CAP-WEBAPP` repair budget = `4/2/2`** (`ALLOWED 4` sau `OWNER_EXTENSION` +2 của
   `DEC-052` §E; `USED 2` = `REPAIR_CYCLE_1` `T-12` + `REPAIR_CYCLE_2` `T-15`; `T-16` và `T-17`
   tiêu 0 mỗi task vì cả hai là implementation ban đầu). Hai chu kỳ còn lại là của cả capability,
   không dành riêng cho task nào. Budget KHÔNG reset — nó chỉ được Owner cấp thêm, có ngày và lý do.
      ...
      └─> WP-A7 ✅ ─┤
                    └─> WP-A6 ✅ (S014) ──> GATE-A ✅ ──> T-06 ✅ ──> WP-B1 ✅ (DONE, DEC-034) ──┐
                                                    (DO_NOT_BUILD, DEC-031)  WP-B2 ✅ (DONE, DEC-038) ─┤
                                                                             WP-B3 ✅ (DONE, DEC-037) ─┘
                                                                                       │
                                                                              GATE-B ✅ (CLOSED, DEC-038) ──> T-07 ✅ (DONE, DEC-040, chọn L-1) ──> T-11 (BLOCKED, not-applicable dưới V2.1.5)
...
T-07 = T-06 DONE ∧ GATE-B — **CẢ HAI ĐÃ THOẢ, `T-07: READY → DONE` tại `DEC-040`** (2026-09-05,
sau phiên SPIKE evidence-investigation `DEC-039`). Owner chọn **L-1** (benchmark đơn giản hơn).
KHÔNG ghi đè verdict official `DO_NOT_BUILD`/`can_proceed_to_app=false` — cả hai giữ nguyên
vĩnh viễn cho V2.1.5.
T-11 vẫn `BLOCKED`: điều kiện `T-07 DONE ∧ WP-C2 ∧ WP-C3 ∧ WP-C4 ∧ verdict=BUILD` nay có `T-07`
và `WP-C2` đã `DONE`, nhưng `WP-C3`/`WP-C4` chưa `DONE` VÀ verdict vẫn `DO_NOT_BUILD` dưới
V2.1.5 — sau `DEC-040`, nhánh V2.1.5 đã đóng lại bằng L-1 nên điều kiện `verdict=BUILD` **không
bao giờ** thoả được nữa dưới tên gọi này; `T-11` trở thành **not-applicable vĩnh viễn dưới
V2.1.5** (chỉ một verdict `BUILD` từ một chiến lược/version khác, tự chạy đủ gate, mới có thể mở
lại điều kiện đó — không kế thừa gì từ V2.1.5).
```

## Current Task Snapshot

**T-14 — DONE (`DEC-050`, Owner Closure, 2026-09-06, sau independent E2 `E2_VERDICT = PASS`).**

    Task ID        T-14
    File           docs/tasks/T-14-buoc-c-firebase-isolation-auth-backup.md
    Báo cáo E2     docs/reviews/T14-E2-INDEPENDENT-REVIEW.md (reviewer khác implementer, HEAD 8ad0f13)
    Báo cáo đóng   docs/reviews/T14-OWNER-CLOSURE.md
    Bằng chứng E2  docs/reviews/evidence/T14-E2/ (37 assertion riêng của reviewer, 0 FAIL)
    Owner Decision DEC-050 (PROJECT/PROJECT_DECISIONS.md)
    Trạng thái     READY -> IN_PROGRESS -> IMPLEMENTED (S039) -> independent E2 PASS (S040) ->
                   IMPLEMENTED -> DONE (S041, Owner-authorized Lifecycle Closure)
    Completion Gate 12/12 REQUIRED PASS — Evidence Level E1 + E2 độc lập cho cả 12
    C-AS           14/14 PASS, tái lập bằng trạng thái/hành động do reviewer kiểm soát
    Production diff của implementation (S039) = +194/−23 trên 4 file, dưới trần +600/−400.
                   Production diff của E2 (S040) + closure (S041) = EMPTY.
    H-42           Phần REQUIRED (FB-2/FB-4/backup-recovery/bằng chứng persistence) ĐÓNG.
                   FB-3 giữ HARDENING (Owner-executed); FB-1 + phần DEFERRED (project riêng)
                   giữ HARDENING, không owner.
    H-49           ĐÓNG. Mô tả chữ ký lỗi legacy sửa theo F-T14-E2-01 (nay hỏng ở SIGNED_OUT,
                   không phải #seedFile — sai lệch mô tả, không phải hành vi).
    H-51 / H-52    Giữ nguyên HARDENING, xác nhận độc lập bởi E2 (F-T14-E2-02); H-52 KHÔNG
                   resolve — vẫn cần Owner tự xác nhận đăng nhập Google thật một lần.
    Finding mới    H-53 (F-T14-E2-03) — Owner CoinDCA đọc users/* của Content; thuộc tính có
                   sẵn từ DEC-023, không phải hồi quy của T-14. Finding, KHÔNG phải task.
    Guard          H-41 VẪN CÒN HIỆU LỰC — T-14 DONE KHÔNG có nghĩa sẵn sàng tiền thật không
                   giới hạn. H-46 vẫn ACTIVE — SELL/realized P&L KHÔNG mở. Bước D
                   (OWNER_LOCAL_ACCEPTANCE) CHƯA mở.
    Integration    branch_authority_check báo INTEGRATION_DECISION_REQUIRED: loc>5000 (do
                   artifact review/evidence, production diff EMPTY). Owner quyết định ACCEPT
                   INTEGRATION/MERGE (DEC-050 §E) — ghi nhận, CHƯA thi hành merge main.
    Owner setup    còn cần: (1) thay OWNER_UID_REQUIRED bằng UID Google thật; (2) firebase
                   target:apply hosting coindca <site-id>; (3) deploy theo runbook 3 bước;
                   (4) tự xác nhận đăng nhập Google một lần trên trình duyệt thật có mạng.
    Budget         CAP-WEBAPP allowed 2 / used 1 / remaining 1 — KHÔNG ĐỔI; S040+S041 tiêu 0 chu kỳ.
    Task ID mới    0 · capability mới 0 · lineage root mới 0 · proposal mới 0

### Snapshot trước đó — T-14 IMPLEMENTED, chờ E2 (giữ để truy vết)

**T-14 — IMPLEMENTED (`S039`, 2026-09-06). Endpoint: `IMPLEMENTED — E2_REQUIRED`. CHƯA `DONE`.**

    Task ID        T-14
    File           docs/tasks/T-14-buoc-c-firebase-isolation-auth-backup.md
    Báo cáo        docs/reviews/T14-IMPLEMENTATION-REPORT.md
    Bằng chứng     docs/reviews/evidence/T14/ (npm-test.log = toàn bộ 8 bước cổng release;
                   pytest.log = 678/678 PASS)
    Phiên thi hành S039 — nhánh claude/t14-step-c-firebase-isolation-xu6nxb, base origin/main 97434d0
    Trạng thái     READY -> IN_PROGRESS -> IMPLEMENTED (Ready Gate 17/17 xác nhận lại trước khi
                   viết dòng production đầu tiên)
    Completion Gate 12/12 REQUIRED PASS — Evidence Level E1 cho cả 12; independent E2 CHƯA chạy
    Production diff +194 / −23 trên 4 file (app_logic.js, ledger_ui.js, firestore.rules,
                   firebase.json). Trần frozen +600/−400 -> TRONG TRẦN.
                   webapp/ledger.js = 0 dòng đổi (LOCKED DEC-042/DEC-043).
                   webapp/app_shell.html = 0 dòng đổi (nút đăng nhập render từ JS).
                   webapp/firebase_config.js = 0 dòng đổi (không thêm secret nào).
    Auth           signInAnonymously() bị gỡ hoàn toàn; onAuthStateChanged là điểm vào danh tính
                   duy nhất; signIn() = signInWithPopup(GoogleAuthProvider); thêm phase SIGNED_OUT.
    Rules          Chỉ comment khối COINDCA. Khối Content + header trùng SHA-256 từng byte với bản
                   trước T-14. Ma trận 5 kịch bản danh tính PASS trên rules thật + token thật.
                   test_shared_rules_merge.js 120 assertion, 0 deviation (baseline DEC-023).
    Deploy         firebase.json hosting.target="coindca"; runbook 3 bước trong webapp/README.md;
                   `firebase deploy` trần bị cấm tường minh.
    Backup/Restore export coindca-ledger-<ISO>.json (schemaVersion + exportedAt + chỉ nguồn sự
                   thật; derivedSnapshot mang nhãn INFORMATIONAL — NOT IMPORTED);
                   restore = preview + validate dry-run -> snapshot coindca-before-restore-<ISO>
                   -> ghi nguyên tử -> ACK. Dị dạng: từ chối trước cả hộp thoại, 0 mutation.
    H-42           Phần REQUIRED đóng ở mức E1 (FB-2, FB-4, backup/recovery, bằng chứng
                   persistence). FB-3 giữ HARDENING; FB-1 giảm thiểu, DEFERRED. H-42 CHƯA đóng.
    H-49           CHECK-T14-11 PASS; sáu file test V2.1.5 nghỉ hưu khỏi cổng release (script
                   test:legacy-v215), thay bằng webapp/test_t14_persistence.js. H-49 CHƯA đóng.
    Hardening mới  H-51 (PRODUCTION_PATHS.md không khai firestore.rules/firebase.json),
                   H-52 (signInWithPopup phụ thuộc apis.google.com). Finding, KHÔNG phải task.
    Guard          H-44…H-48, H-50 nguyên trạng. H-46 vẫn ACTIVE — SELL/realized P&L KHÔNG mở.
                   Cảnh báo H-41 ("dừng dùng tiền thật không giới hạn") VẪN CÒN HIỆU LỰC.
    Budget         CAP-WEBAPP allowed 2 / used 1 / remaining 1 — KHÔNG ĐỔI; S039 tiêu 0 chu kỳ.
    Task ID mới    0 · capability mới 0 · lineage root mới 0 · proposal mới 0

Chuyển `IMPLEMENTED → DONE` thuộc thẩm quyền chủ dự án (`STATE_AUTHORITY.md`), sau independent E2
— đúng tiền lệ `T-09B`/`T-12`/`T-13`.

### Snapshot trước đó — T-14 READY (giữ để truy vết)
...
Chưa có phiên thi hành *tại thời điểm `S038`*. `T-14 READY` không có nghĩa Firebase/auth/rules/
backup đã đổi — chỉ có nghĩa kiến trúc đã đủ rõ để một phiên thi hành riêng bắt đầu mà không cần
hỏi lại Owner các lựa chọn kỹ thuật đã chốt. Phiên thi hành đó là `S039` (xem snapshot ở trên).
...
**T-12 — DONE (`DEC-046`, Owner Closure, 2026-09-05, sau independent E2 PASS).**

    Task ID        T-12
    File           docs/tasks/T-12-so-cai-l1-v2-va-derive.md
    Task Mode      MAJOR (đã đóng — không có task MAJOR nào đang thi hành kế tiếp)
    Trạng thái     DONE — Completion Gate 14/14 REQUIRED PASS (5 E1-only + 9 E1+E2)
    Routing        D / max; model_score3.1, effort_score3.65
    Gate           FROZEN2026-09-05; 14/14 PASS, không sửa câu chữ/ngữ nghĩa
    Ready Gate     Tái đánh giá duy nhất sauDEC-045: 17/17PASS
    Capability     CAP-WEBAPP, lineage rootWP-C1
    Kiến trúc      L-1 trong ethdca/state, không đổi Firebase/auth/rules/hosting
    Golden         c610a299ed6b66dea3cd63372a0943967c93e95d — frozen, không đổi trong suốt E2
    Repair         BASEc610a29 → HEAD2a2ab3f; REPAIR_CYCLE_1 CONSUMED (không đổi bởi closure)
    Budget         allowed2/used1/remaining1 — E2 và Owner closure KHÔNG tiêu chu kỳ nào
    Evidence       SC12/12, INV15/15, mutation7/7, P1…P6 E1 + 9/9 check E2-required PASS độc lập
    Regression     Python678/678 PASS; npm legacy N/A từng ca, persistence L-1 PASS
    E2 report      docs/reviews/T12-E2-INDEPENDENT-REVIEW.md — E2_VERDICT = PASS, 0 BLOCKING
    Findings       F-E2-01…04 → HARDENING H-44…H-47, kèm RE_TRIGGER_CONDITION, không task mới
    Guard          T-12 DONE KHÔNG cấp phép SELL tiền thật — H-46/F-E2-03 phải xử lý TRƯỚC


Bước B/C/D chưa mở — quyết định bởi Owner, không phải hệ quả tự động của DONE này. `H-42` (bước
C) và `OWNER_LOCAL_ACCEPTANCE` (bước D) chưa đóng; `H-46` (`F-E2-03`) phải xử lý trước SELL thật.
Không ghi hay đọc dữ liệu tài chính thật của Owner. Nguồn checkpoint:
`7d1985aaf306294df49c9508078d5425da10f47e`; production diff của E2/closure = 0.
...
spec đó tồn tại. (Điều kiện này **đã thoả** tại `DEC-042`, 2026-09-05.)
...
- **DEC-042** — DUYỆT CoinDCA L-1 Product + Accounting Spec `CANONICAL — APPROVED`; bốn quyết
  định kế toán `OD-L1-1`…`OD-L1-4` chốt. Task ID mới = 0
- **DEC-043** — tu chỉnh `T-12` (persistence architecture bounded, tách golden baseline,
  pre-authorize một repair cycle có điều kiện)
- **DEC-044/045** — `T-12`: sửa oracle carry SC-04; WAC sau ROUND_VND và carry tháng đã đóng
- **DEC-046** — Owner-authorized Lifecycle Closure: `T-12: IMPLEMENTED → DONE`, sau independent
  E2 `PASS`. `CAP-WEBAPP` budget `2/1/1` không đổi
...
- **DEC-050** — Owner-authorized Lifecycle Closure: `T-14: IMPLEMENTED → DONE`, sau independent
  E2 `E2_VERDICT = PASS` (`docs/reviews/T14-E2-INDEPENDENT-REVIEW.md`). Completion Gate 12/12
  REQUIRED PASS (E1+E2); 14/14 `C-AS` PASS. `H-42` phần REQUIRED ĐÓNG; `H-49` ĐÓNG (mô tả sửa
  theo `F-T14-E2-01`); `H-51`/`H-52` xác nhận, giữ HARDENING (không resolve); một finding mới
  `H-53`, 0 BLOCKING. `CAP-WEBAPP` budget `2/1/1` không đổi. Integration Decision: **ACCEPT
  INTEGRATION/MERGE** cho `INTEGRATION_DECISION_REQUIRED: loc>5000` (artifact review/evidence,
...
- **DEC-053** — Owner Direction + Lifecycle Closure: mở và đóng `T-17` (sắp xếp lại giao diện
  CoinDCA L-1 — Tổng quan theo nhóm, bộ lọc Lịch sử dạng nút có đếm, icon/màu theo loại giao
  dịch) trong một phiên; thuần trình bày, `webapp/ledger.js` diff RỖNG nên KHÔNG mang category
  `accounting_financial` (khác `T-12`/`T-13`/`T-16`) — routing `D2 R2 B2 A1 X1` → Tier B/Sonnet,
  `U1 V3 H2 C2 F3` → effort `high`. Completion Gate 14/14 REQUIRED PASS (E1); toàn bộ `npm test`
  (10 suite, gồm emulator + trình duyệt thật) PASS nguyên văn, không sửa test nào. Ghi nhận quyết
  định duy nhất: giữ `#histFilterType` (select ẩn đồng bộ giá trị) thay vì đổi hẳn sang nút, vì
...
- **S041 (2026-09-06)** — **OWNER CLOSURE `T-14`** (chỉ thị phiên trực tiếp "COINDCA — T-14 OWNER
  CLOSURE + INTEGRATION DECISION", nhánh `claude/t14-step-c-firebase-isolation-xu6nxb`).
  `T-14: IMPLEMENTED → DONE` (`DEC-050`). Xác nhận lại điều kiện đóng từ canonical state (không
  chạy lại toàn bộ E2 — canonical state nhất quán): Ready Gate 17/17, Completion Gate **12/12
  REQUIRED PASS ở mức E1+E2**, 14/14 `C-AS` PASS, accounting non-regression PASS, T-13
  non-regression PASS, 0 BLOCKING, production diff của E2+closure = EMPTY. `H-42` phần REQUIRED
  **ĐÓNG**; `H-49` **ĐÓNG** (mô tả chữ ký lỗi legacy sửa theo `F-T14-E2-01` — nay hỏng ở
  `SIGNED_OUT`, không phải `#seedFile`); `H-51`/`H-52` xác nhận độc lập, giữ nguyên HARDENING,
  **không resolve** `H-52`; finding mới `H-53` (`F-T14-E2-03`, quan sát không hồi quy). Guard
  giữ nguyên: `H-41` vẫn còn hiệu lực, `H-46` vẫn ACTIVE, bước D chưa mở. **Integration Decision**:
  `branch_authority_check` báo `INTEGRATION_DECISION_REQUIRED: loc>5000` (do artifact review/
  evidence tích luỹ qua `S039`+`S040`+`S041`, production diff EMPTY) — Owner quyết định **ACCEPT
  INTEGRATION/MERGE**, ghi nhận tại `DEC-050` §E, **không** merge `main` trong phiên này.
  `CAP-WEBAPP` budget **KHÔNG đổi** (`2/1/1`) — S041 tiêu **0** repair cycle. Không sửa mã
  production. Task ID mới = 0, capability mới = 0, lineage root mới = 0. `docs/reviews/
  T14-OWNER-CLOSURE.md`, `DEC-050`.

- **S040 (2026-09-06)** — **Independent E2 review** cho `T-14` (reviewer khác implementer, HEAD
  reviewed `8ad0f13`, nhánh `claude/t14-step-c-firebase-isolation-xu6nxb`) → `E2_VERDICT = PASS`.
  Reviewer xử lý mọi kết luận của `T14-IMPLEMENTATION-REPORT.md` như CLAIM cần kiểm, tự viết 37
  assertion độc lập (20 ma trận phân quyền + 17 ngữ nghĩa restore/derive) không tái sử dụng
  assertion của implementer, và chạy lại `npm --prefix webapp test` (exit 0) trong môi trường
  của chính mình. Xác nhận độc lập: SHA-256 phần thi hành của `firestore.rules` trùng khít trước/
  sau `T-14` (chỉ comment đổi); `webapp/ledger.js` + golden fixture + `engine.js`/`app_shell.html`/
  `build_app.js` = 0 dòng đổi; SC-01…12 + INV-1…15 PASS; 7/7 mutant bị diệt; Content safe-merge
  120 assertion, 0 deviation. `H-52` xác nhận là giới hạn môi trường (đo trực tiếp
  `apis.google.com` HTTP `000`; không nhánh rẽ theo provider trong mã production). `H-51` xác
  nhận là finding thật, giữ HARDENING. Ba finding mới `F-T14-E2-01`/`02`/`03` (routed `H-49`/
  `H-51`/mục mới), 0 BLOCKING. Production diff của phiên review = **EMPTY**. Không sửa mã
  production, không tiêu repair authority. `docs/reviews/T14-E2-INDEPENDENT-REVIEW.md`,
  `docs/reviews/evidence/T14-E2/`.

- **S039 (2026-09-06)** — **THI HÀNH `T-14`** (`READY → IN_PROGRESS → IMPLEMENTED`), nhánh
  `claude/t14-step-c-firebase-isolation-xu6nxb` tách đúng từ `origin/main` `97434d0`. Ready Gate
  17/17 được xác nhận lại TRƯỚC dòng mã production đầu tiên. **12/12 REQUIRED check PASS ở mức
  E1**; independent E2 CHƯA chạy → endpoint `IMPLEMENTED — E2_REQUIRED`, **không** chuyển `DONE`.
  Nội dung: (1) danh tính — gỡ hoàn toàn Anonymous Auth, `onAuthStateChanged` là điểm vào danh
  tính duy nhất, `signIn()` = `signInWithPopup(GoogleAuthProvider)`, thêm phase `SIGNED_OUT`, nút
  đăng nhập/đăng xuất render từ JS nên `app_shell.html` **0 dòng đổi**; (2) rules — `firestore.rules`
  chỉ đổi comment trong khối `COINDCA`, header + TOÀN BỘ khối Content trùng **SHA-256 từng byte**
  với bản trước T-14, ma trận 5 kịch bản danh tính PASS trên rules thật với token thật do Auth
  Emulator ký, `test_shared_rules_merge.js` 120 assertion / 0 deviation (baseline `DEC-023`);
  (3) deploy — `firebase.json` khai `hosting.target: "coindca"`, runbook 3 bước
  (`test:rules-merge` → đọc diff → deploy) trong `webapp/README.md`, cấm `firebase deploy` trần;
  (4) backup/recovery — export `coindca-ledger-<ISO>.json` có `schemaVersion` + `exportedAt` +
  chỉ nguồn sự thật (`derivedSnapshot` mang nhãn `INFORMATIONAL — NOT IMPORTED`), restore có
  preview/validate dry-run TRƯỚC hộp thoại, snapshot `coindca-before-restore-<ISO>`, ghi nguyên
  tử, backup dị dạng bị từ chối với **0 mutation bền** (4 ca, so bit-for-bit). `H-49` hấp thụ:
  `webapp/test_t14_persistence.js` (14 kịch bản, 60 assertion qua UI Step B) thay sáu file test
  V2.1.5 — sáu file được **giữ nguyên trên đĩa**, chuyển sang script `test:legacy-v215`, KHÔNG
  resurrect UI V2.1.5; `npm --prefix webapp test` **exit 0**. Regression: `test_t12_ledger`,
  `test_t12_mutations` (7/7 mutant bị diệt), `test_t12_browser` (17/17), `test_stepb_ui` đều PASS;
  `webapp/ledger.js` và golden fixture **0 dòng đổi**. Production diff **+194/−23 trên 4 file**
  (trần frozen +600/−400) → TRONG TRẦN, không `CHANGE_BUDGET_EXCEEDED`. Không hard-stop nào kích
  hoạt. Hai finding mới `H-51`/`H-52` (finding, KHÔNG phải task). `H-44`…`H-48`, `H-50` nguyên
  trạng; `H-46` vẫn ACTIVE (SELL không mở); cảnh báo `H-41` vẫn còn hiệu lực. `CAP-WEBAPP` budget
  **KHÔNG đổi** (`2/1/1`) — `S039` tiêu **0** repair cycle. Task ID mới = 0, capability mới = 0,
  lineage root mới = 0, proposal mới = 0. `docs/reviews/T14-IMPLEMENTATION-REPORT.md`,
  `docs/sessions/S039-t14-step-c-firebase-isolation-implementation.md`,
  `docs/reviews/evidence/T14/`.
...
- **S035 (2026-09-05)** — MỞ `T-13` (CoinDCA L-1 Bước B: Dashboard hằng ngày + Nhập giao
  ...
  ghi nhận thành `DEC-047`. Sản phẩm: `docs/spec-l1/COINDCA_L1_STEP_B_UX_SPEC.md`
  (`CANONICAL — APPROVED`, mới) + `docs/tasks/T-13-buoc-b-dashboard-giao-dich-lich-su.md` (mới,
  `NOT_PLANNED → READY`, Completion Gate 13/13 REQUIRED FROZEN, routing `C/Opus/xhigh`).
  Capability `CAP-WEBAPP`, lineage root `WP-C1`, budget KHÔNG đổi (`2/1/1`). Task ID mới = 1;
  capability/lineage mới = 0. Production diff = **EMPTY**. Không thi hành, không `IN_PROGRESS`.
  `docs/sessions/S035-l1-step-b-task-definition.md`.
...
**Hiện hành sau `S041` (Owner Closure `T-14`):**

    NEXT SMALLEST ACTION = KHÔNG có phiên implementation nào đang chờ. `T-14 DONE`. Hai việc còn
    mở, cả hai đều thuộc thẩm quyền/thao tác Owner, không phải phiên agent kế tiếp tự khởi động:
    (1) thiết lập thật (UID + deploy + xác nhận đăng nhập Google — xem dưới); (2) quyết định
    merge nhánh `claude/t14-step-c-firebase-isolation-xu6nxb` vào `main` (Owner đã quyết định
    ACCEPT INTEGRATION tại `DEC-050` §E nhưng chưa thi hành — merge là hành động riêng, tách khỏi
    việc đóng lifecycle).

`T-14` đã **DONE** (`DEC-050`, 2026-09-06, `S041`). Completion Gate 12/12 REQUIRED PASS ở mức
E1+E2; `H-42` phần REQUIRED và `H-49` đã ĐÓNG. Ràng buộc cho bất kỳ phiên nào chạm capability
`CAP-WEBAPP` tiếp theo:

1. Chạy `branch_authority_check.sh` TRƯỚC khi đọc state.
2. `CAP-WEBAPP` không có task nào đang mở. Một task mới trong capability này (nếu có) dùng chung
   pool budget hiện tại (`allowed 2 / used 1 / remaining 1`), không được reset.
3. Guard giữ nguyên tuyệt đối: `H-46` (SELL) chưa có Owner Decision, guard giữ ACTIVE; bước D
   (`OWNER_LOCAL_ACCEPTANCE`) chưa mở — chỉ có ý nghĩa sau khi Owner hoàn tất thiết lập thật;
   cảnh báo `H-41` ("dừng dùng app với tiền thật không giới hạn") **vẫn còn hiệu lực** — `T-14
   DONE` KHÔNG đồng nghĩa sẵn sàng tiền thật không giới hạn.
4. `H-42` phần DEFERRED (project Firebase riêng), `FB-1`, `FB-3`, `H-51`, `H-52`, `H-53` vẫn
   HARDENING trong backlog, không owner, chờ Owner chủ động — không tự đóng bởi bất kỳ phiên
   agent nào.
5. **Owner setup còn cần trước khi dùng thật** (không phải hệ quả tự động của `T-14 DONE`,
   không thực hiện bởi agent trừ khi Owner uỷ quyền tường minh):
   - thay `OWNER_UID_REQUIRED` bằng UID Google thật trong `firestore.rules`, deploy theo đúng
     runbook 3 bước (`npm --prefix webapp run test:rules-merge` → đọc diff → deploy có điều
     kiện; không bao giờ `firebase deploy` trần);
   - chạy một lần `firebase target:apply hosting coindca <site-id-coindca>`;
   - tự xác nhận đăng nhập Google thành công một lần trên trình duyệt thật có mạng (đóng `H-52`
     nếu thành công).
6. **Integration**: `branch_authority_check` báo `INTEGRATION_DECISION_REQUIRED: loc>5000`.
   Owner đã quyết định **ACCEPT INTEGRATION/MERGE** (`DEC-050` §E) nhưng merge `main` **chưa**
   được thi hành trong bất kỳ phiên nào — đó là một hành động tích hợp riêng, cần một phiên/uỷ
   quyền riêng, không tự động theo sau việc đóng lifecycle.

### Ràng buộc đã dùng cho phiên thi hành `T-14` (đã hoàn tất tại `S039` — giữ để truy vết)

`T-14` khi đó **READY** (`DEC-049`, 2026-09-06, phiên định nghĩa `S038`). Ràng buộc lúc đó:
...
1. Đọc theo thứ tự: `AGENTS.md` → `governance/v4/CORE/*` → `PROJECT/PROJECT_PROFILE.md`,
   ...
   (§2.2.9), `HARDENING_BACKLOG.md` (`H-42`, `H-45`, `H-46`), `PROJECT_DECISIONS.md` (`DEC-042`,
   `DEC-046`, `DEC-047`) → `docs/tasks/T-13-buoc-b-dashboard-giao-dich-lich-su.md` →
   `docs/spec-l1/COINDCA_L1_STEP_B_UX_SPEC.md` → `docs/spec-l1/COINDCA_L1_PRODUCT_ACCOUNTING_SPEC.md`.
2. Chạy `branch_authority_check.sh` TRƯỚC khi đọc state; nhánh mới tách từ `origin/main`.
3. Completion Gate của `T-13` đã **đóng băng** 2026-09-05 — không xoá/làm yếu REQUIRED check;
   thay đổi phải qua `COMPLETION GATE CHANGE PROPOSAL`.
4. **Không** mở thêm task ID nào. **Không** tự cấp repair budget — `CAP-WEBAPP` còn đúng 1 chu kỳ
   `REMAINING` (không pre-authorized cho `T-13`, khác `T-12`/`DEC-043`); nếu cần, đó là một Owner
   Decision riêng khi một REQUIRED check FAIL thật sự.
5. Mọi con số hiển thị PHẢI đến từ `CoinLedger.derive()`; mọi ghi dữ liệu PHẢI qua
   `CoinLedger.update()/migrate()/destructive()` — không viết công thức tài chính mới trong UI
   (`O-8` của file task).
6. SELL bị ẩn hoàn toàn khỏi mọi form/menu (`S-B10`); không chạm `firebase.json`/
   `firestore.rules`/`webapp/firebase_config.js` (`O-2`, bước C).
...
**Cập nhật sau `S033` (2026-09-05) — Owner Decision `DEC-043` tu chỉnh `T-12`.**
    ...
    NEXT SMALLEST ACTION = mở một phiên thi hành RIÊNG cho T-12.

Điều kiện của phiên đó:

1. Đọc theo thứ tự: `AGENTS.md` → `governance/v4/CORE/*` → `PROJECT/PROJECT_PROFILE.md`,
   `CAPABILITY_REGISTRY.md`, file này, `PRODUCTION_PATHS.md`, `REVIEW_BUDGET_LEDGER.md`,
   `HARDENING_BACKLOG.md` (`H-41`, `H-42`, `H-43`), `PROJECT_DECISIONS.md` (`DEC-040`,
   `DEC-041`, `DEC-042`, `DEC-043`) → `docs/tasks/T-12-so-cai-l1-v2-va-derive.md` →
   `docs/spec-l1/COINDCA_L1_PRODUCT_ACCOUNTING_SPEC.md`.
2. Chạy `branch_authority_check.sh` TRƯỚC khi đọc state; nhánh mới tách từ `origin/main`.
3. Completion Gate của `T-12` đã **đóng băng** 2026-09-05 và **không đổi bởi `DEC-043`** —
   không xoá/làm yếu REQUIRED check; thay đổi phải qua `COMPLETION GATE CHANGE PROPOSAL`.
4. **Không** mở thêm task ID nào. **Không** tự cấp repair budget NGOÀI chu kỳ pre-authorized của
   `DEC-043` (và chu kỳ đó chỉ dùng được khi đủ điều kiện đã liệt kê trong file task — tự đối
   chiếu, ghi rõ trong evidence). **Không** tự chứng nhận `E2`.
5. `E2` cho khối tính đúng tài chính do một phiên reviewer **độc lập** tạo, sau khi thi hành
   xong — không phải trong cùng phiên. Yêu cầu `E2` KHÔNG đổi bởi `DEC-043`.
6. Sổ L-1 phải nằm **trong** `ethdca/state` đã có (`DEC-043` mục A); cần collection/document mới
   hoặc sửa `firestore.rules` ⇒ dừng với `ARCHITECTURE_CHANGE_REQUIRED`, không tự suy diễn rộng
   thẩm quyền của `DEC-043`.
7. `T12_GOLDEN_ACCOUNTING_BASELINE` (tổng hợp) là đủ cho gate kế toán; **không** cần dữ liệu tài
   chính thật của Owner để bắt đầu hay hoàn tất `T-12` (`DEC-043` mục B).

Cần Owner quyết định (khi tới lúc, không chặn việc bắt đầu `T-12`):
- chu kỳ repair **thứ hai** cho `T-12` nếu chu kỳ pre-authorized đã dùng mà vẫn FAIL
  (`CAPABILITY_MODEL.md` §II.8; `DEC-043` không tự động mở rộng);
- `IMPLEMENTED → DONE` của `T-12`.
